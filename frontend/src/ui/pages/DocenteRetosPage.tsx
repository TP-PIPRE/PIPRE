...
const MOCK_COURSES = [
  { id_course: "1", name: "Robótica Nivel 1: Fundamentos (Local)", level: "Básico" },
  { id_course: "2", name: "Programación de Microcontroladores (Local)", level: "Intermedio" },
  { id_course: "3", name: "Diseño y Mecánica de Robots (Local)", level: "Avanzado" },
];
...
export const DocenteRetosPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({ name: "", description: "", level: "BASIC" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (modalType === "create") {
        ...
        await apiService.courses.update({ ...formData, id_course: selectedCourse.id_course });
      }
      setModalType(null);
      fetchCourses();
    } catch (err) {
  ...
  const handleDelete = async () => {
    if (selectedCourse) {
      try {
        ...
        setCourses(prev => prev.filter(c => c.id_course !== selectedCourse.id_course));
      } catch (err) {
  ...
  return (
    <main className="flex-1 p-8 max-w-7xl mx-auto w-full animate-fade-in-soft">
      <header className="mb-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        ...
        <button 
          onClick={() => { setModalType("create"); setFormData({ name: "", description: "", level: "BASIC" }); }}
            ...
            <div className="relative h-32 bg-primary/10 overflow-hidden flex items-center justify-center border-b border-border/10">
              ...
              <div className="relative text-center">
                <span className="text-[9px] font-black text-primary uppercase tracking-[0.4em] mb-2 block animate-fade-in-soft">
                  {modalType === "create" ? "Configuración de Nodo" : "Modificación de Arquitectura"}
                </span>
                <h2 className="text-2xl font-bold tracking-tight">
                  {modalType === "create" ? "Nuevo Reto Educativo" : "Editar Especificaciones"}
                </h2>
              </div>
            </div>
          )}
            ...
            {modalType === "delete" ? (
              <div className="text-center py-6">
                ...
                <p className="text-sm text-text-muted/70 mb-10 px-6 leading-relaxed">
                  Esta acción desactivará permanentemente el nodo <span className="text-text font-bold">"{selectedCourse?.name}"</span>. Esta operación es irreversible.
                </p>
                <div className="flex gap-4">
                  <button onClick={() => setModalType(null)} className="flex-1 btn-secondary py-4 text-[10px] font-bold uppercase tracking-widest">
                    CANCELAR
                  </button>
                  <button onClick={handleDelete} className="flex-1 bg-danger text-white hover:brightness-110 py-4 text-[10px] font-bold uppercase tracking-widest shadow-xl shadow-danger/20 transition-all active:scale-95" style={{ borderRadius: "var(--theme-radius)" }}>
                    ELIMINAR NODO
                  </button>
                </div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2 md:col-span-2">
                    <label className="text-[10px] uppercase tracking-widest text-text-muted/60 font-black ml-1">Identificador del Curso</label>
                    <input 
                      required
                      value={formData.name}
                  ...
                  <div className="space-y-2 md:col-span-2">
                    <label className="text-[10px] uppercase tracking-widest text-text-muted/60 font-black ml-1">Nivel de Complejidad</label>
                    <div className="relative">
                      <select 
                        ...
                        style={{ borderRadius: "var(--theme-radius)" }}
                      >
                        <option value="BASIC">Protocolo Básico (Fundamentos)</option>
                        <option value="INTERMEDIATE">Protocolo Intermedio (Control)</option>
                        <option value="ADVANCED">Protocolo Avanzado (Sistemas)</option>
                      </select>
                      <span className="material-symbols-outlined absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-text-muted/40">expand_more</span>
                    </div>
                  </div>

                  <div className="space-y-2 md:col-span-2">
                    <label className="text-[10px] uppercase tracking-widest text-text-muted/60 font-black ml-1">Descripción Técnica</label>
                    <textarea 
                      required
                ...
                <div className="flex gap-4 pt-4">
                  ...
                  >
                    {modalType === "create" ? "SINCRONIZAR NODO" : "GUARDAR CAMBIOS"}
                  </button>
                </div>
              </form>
            )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div className="relative z-10">
              <div className="flex justify-between items-start mb-8">
                ...
                <div className="flex gap-2">
                  <button 
                    onClick={() => { setModalType("edit"); setSelectedCourse(course); setFormData({ name: course.name, description: "Módulo industrial.", level: "BASIC" }); }}
                  ...
                  <button 
                    onClick={() => { setModalType("delete"); setSelectedCourse(course); }}
              ...
              <div className="flex gap-4 pt-6 border-t border-border/10">
                <button className="flex-1 text-[10px] font-black uppercase tracking-widest py-3 bg-bg/60 border border-border/10 hover:border-primary/30 hover:bg-primary/5 transition-all active:scale-95" style={{ borderRadius: "var(--theme-radius)" }}>
                  Módulos
                </button>
                <button className="flex-1 text-[10px] font-black uppercase tracking-widest py-3 border border-border/10 hover:border-primary/30 transition-all active:scale-95" style={{ borderRadius: "var(--theme-radius)" }}>
                  Detalles
                </button>
              </div>
            </div>
        ...
        >
          ...
          <span className="text-[10px] font-bold uppercase tracking-[0.3em] opacity-60 group-hover:opacity-100 transition-opacity">Añadir Nuevo Nodo</span>
        </div>
      </div>
    </main>
  );
