...
class RIA06Service:
    ...
    def set_model(self, model) -> None:
        if not getattr(model, "is_fitted", False):
            raise ValueError("El modelo RIA06 persistido no está entrenado.")
        if getattr(model, "model_version", None) != self.MODEL_VERSION:
            raise ValueError("El modelo RIA06 persistido tiene una versión incompatible.")
        expected_schema = self.model.FEATURE_SCHEMA_VERSION
        if getattr(model, "feature_schema_version", None) != expected_schema:
            raise ValueError(
                "El modelo RIA06 persistido tiene un esquema incompatible."
            )
        self.model = model
        self._trained = True
